pipeline {
  ...
  stages {
    ...
    stage('Small dataset') {
      steps {
        sh 'cd /data_gms_somatic && snakemake -j 1 -s /Twist_DNA/gms_somatic.smk --directory /data_gms_somatic --use-singularity --singularity-args "--bind /beegfs-storage --bind /data --bind /Twist_DNA --bind /data_gms_somatic"'
