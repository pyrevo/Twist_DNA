...
header = ['Sample','Total reads','Reads aligned [%]','Duplicates [%]','Bases on target [%]','Target bases over 100X [%]','Mean target coverage','Median target coverage','Median insert size','Insert size s.d.','AT-dropout [%]','GC-dropout [%]','Target bases with zero coverage [%]','FOLD-80']
line = [sample, metricsDict3['TOTAL_READS'], 100*float(metricsDict3['PCT_PF_READS_ALIGNED']),100*float(samDict['reads duplicated']) / float(samDict['raw total sequences']), 100*float(metricsDict1['PCT_SELECTED_BASES']), 100*float(metricsDict1['PCT_TARGET_BASES_100X']), metricsDict1['MEAN_TARGET_COVERAGE'], metricsDict1['MEDIAN_TARGET_COVERAGE'], metricsDict2['MEDIAN_INSERT_SIZE'], metricsDict2['STANDARD_DEVIATION'], 100*float(metricsDict4['AT_DROPOUT']), 100*float(metricsDict4['GC_DROPOUT']), 100*float(metricsDict1['ZERO_CVG_TARGETS_PCT']), metricsDict1['FOLD_80_BASE_PENALTY']]
